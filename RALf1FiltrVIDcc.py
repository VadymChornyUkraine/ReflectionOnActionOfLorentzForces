            ...
            if ann==0: 
                ...
                if sum(np.abs(dd)==np.Inf)==0:   
                    ...
                    for l in range(NChan):   
                        ...
                        r2[hh,Nf-NNew+Nf*l:Nf+Nf*l]=r2[hh,Nf-NNew+Nf*l:Nf+Nf*l]-np.mean(arr_c[hh,Nf-NNew+Nf*l:Nf-NNew0+Nf*l])+np.mean(arr_c[hh-1,Nf-NNew-1+Nf*l:Nf-NNew0-1+Nf*l])
        ...
        while hh<Nhh:
            if hh<Nhh:       
                ...
                if (sum(np.abs(arr_bbbxxx)==np.Inf)==0 and sum(np.isnan(arr_bbbxxx))==0):                
                    ...
                    arr_c_=[]
                    for l in range(NChan):
                        dd_=arr_bbbxxx[Nf-1-NNew0+Nf*l:Nf-NNew0_+Nf*l:-1].copy()
                        arr_c_.append(dd_)  
